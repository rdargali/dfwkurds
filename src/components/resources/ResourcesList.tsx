...
export function ResourcesList({ resources, locale }: ResourcesListProps) {
  ...
  // Get categories that have resources, in order
  const activeCategories = categoryOrder.filter(cat => groupedResources[cat]?.length > 0)
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="text-center md:text-start">
              ...
              <p className="text-slate-600">
                {locale === 'ckb'
                  ? 'ئەگەر پرسیارت هەیە دەربارەی هەر سەرچاوەیەک، پەیوەندیمان پێوە بکە'
                  : locale === 'kmr'
                    ? 'Ger pirsên we li ser her çavkaniyê hebin, bi me re têkilî daynin'
                    : 'If you have questions about any resource, contact us'}
              </p>
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <a
                ...
                className="btn bg-slate-200 hover:bg-slate-300 text-slate-700 btn-touch"
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path
                    strokeLinecap="round"
                ...
                {locale === 'ckb' ? 'ئیمەیڵ' : locale === 'kmr' ? 'E-name' : 'Email'}
              </a>
            </div>
          </div>
        </div>
