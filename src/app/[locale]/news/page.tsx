...
import { getSanityData, type NewsPost } from '@/lib/sanity'
...
export default async function NewsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  try {
    newsPosts = await getSanityData<NewsPost[]>(NEWS_POSTS_QUERY)
    // Log for debugging in production
    if (process.env.NODE_ENV === 'production') {
      console.log(`[News] Fetched ${newsPosts.length} posts from Sanity`)
    }
  } catch (error) {
    console.error('[News] Error fetching from Sanity:', error)
    console.log('[News] Using placeholder news posts data')
  }

  // Use placeholder if no data
  const displayPosts = newsPosts.length > 0 ? newsPosts : placeholderNewsPosts
